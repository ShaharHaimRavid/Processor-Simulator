...
void cache_flush(cache_t *c, uint8_t index_of_block)
{
	...
	main_memory_write(c->bus, addr, c->data[index_of_block]);
}
...
void cache_snoop(bus_origid_t origid, bus_command_t cmd, bus_addr_t addr, uint32_t data, bool_t shared, void *user_data)
{
	...
	case MESI_MODIFIED:
		if (cmd == BUS_COMMAND_READ)
		{
			...
			c->metadata[index_of_block] = tag | MESI_SHARED << 12;
		}
		else if (cmd == BUS_COMMAND_READX)
		{
			cache_flush(c, index_of_block);
			c->metadata[index_of_block] = tag | MESI_INVALID << 12;
		}
		break;
	case MESI_EXCLUSIVE:
		if (cmd == BUS_COMMAND_READ)
...
bool_t cache_read(cache_t *c, uint32_t addr, uint32_t *data)
{
	...
	bool_t shared;
	main_memory_read(c->bus, addr, BUS_COMMAND_READ, &block_from_memory, &shared);
...
void cache_write(cache_t *c, uint32_t addr, uint32_t data)
{
	...
	if (!found || mesi == MESI_INVALID || mesi == MESI_SHARED)
	{
		block block_from_memory;
		bool_t shared;
		main_memory_read(c->bus, addr, BUS_COMMAND_READX, &block_from_memory, &shared);
